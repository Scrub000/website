...
class Create(Base):
    slug = wtforms.StringField(
        label="Slug",
        description="Slugs should be in this format: this-is-a-slug",
        validators=[
            validators.InputRequired(message="Please enter a slug."),
            validators.Length(
                min=4,
                max=200,
                message="Slug must be between 4 and 200 characters long.",
            ),
            custom_validators.IUnique(
                model=models.Category,
                field=models.Category.slug,
                message="This slug already exists.",
            ),
            custom_validators.Slug(
                model=models.Category,
                field=models.Category.slug,
