...
def get_confirm_email_token(account: models.Account, expires_in: int = 600):
    """
...
def get_reset_password_token(account: models.Account, expires_in: int = 600):
    """
...
def verify_confirm_email_token(token: str):
    """
...
def _verify_token(token: str, key_type: str):
    try:
        pk = jwt.decode(
            jwt=token,
